...
// Fyers spotIndex after Authentication WEB Socket call to backed Python local Web Socket 
// needed due to SS: L Incalid Vertificate issue 
// cannot issue new https.Agent({
 // rejectUnauthorized:false
// });    fetch(url,{ agent });  
router.get('/fyerslocallive', async function (req,res) {

    let s = ''
	let code = ''
	let auth_code= '';
	if( req.query !== null && req.query !== undefined ){
		console.log(" FYERS LOCAL Python WebSocket PARAMS " +JSON.stringify(req.query))
		var queryJSON  = JSON.parse(JSON.stringify(req.query));
		s = queryJSON['s'];
		  code =queryJSON['accessToken'];
		 
		 console.log(`accessToken : ${code}   `);
		//  const pythonUrl =
        //    `https://localhost:9384/stream?accessToken=${code}`;
		  const pythonUrl =
            `https://fyersfeed.onrender.com/stream?accessToken=${code}`;

            console.log("Connecting to Python SSE:", pythonUrl);
		try {
					...
					if (!response.ok) {
					throw new Error("Python stream connection failed");
					}
					/*
					Setup SSE response for React
					*/
					res.writeHead(200, {
					...
					console.log("Python stream ended");
					res.end();
					});
		 ...
		 console.log("  Fyers is with not accessToken , or could not PARSE THEM ")
	if(res.data !== null && res.data !==undefined){
		s = res .data['s'];
		  code = res .data['code'];
		 auth_code= res .data['auth_code'];
...
// QUICK LOGIN with self triggered  button click
router.get('/fyerstruedatalive', async function (req,res) {

    let s = ''
	let code = ''
	let auth_code= '';
	if( req.query !== null && req.query !== undefined ){
		console.log(" FYERS TRUE DATA QUERY PARAMS " +JSON.stringify(req.query))
		var queryJSON  = JSON.parse(JSON.stringify(req.query));
		s = queryJSON['s'];
		  code =queryJSON['code'];
		 auth_code= queryJSON['auth_code'];
		...
		console.log("FYERS TRUE DATA PARAMS : "+ JSON.stringify(req.params))
		const user = 'FYERS2334'
		const pwd = 'KdRi5X55'
		const port = 8082
		const symbols = ['NIFTY-I', 'BANKNIFTY-I', 'CRUDEOIL-I']; // symbols in array format
		let  newSymbols = ['NIFTY 50', 'NIFTY25093025200CE', 'NIFTY25093025200PE' , 'NIFTY25093025100CE', 'NIFTY25093025100PE', 'NIFTY25093025300CE' , 'NIFTY25093025300PE'];

		//wss://push.truedata.in:8082?user=FYERS2334&password=KdRi5X55"
		...
		rtFeed.on('tick', tickHandler); // Receives Tick data
	      // Dynamically subscribe to new symbols // newSymbols is array of symbol
